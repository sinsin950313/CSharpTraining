using System;
using System.Collections.Generic;
using System.Data.SqlTypes;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace A085_DateTime
{
    internal class Program
    {
        static void Main(string[] args)
        {
            DateTime date1 = new DateTime(1992, 7, 4, 8, 44, 0);
            DateTime date2 = new DateTime(1990, 1, 27, 12, 6, 0);

            Console.WriteLine(date1);
            Console.WriteLine(date2);

            Console.WriteLine("{0}과 {1}의 차이는 {2}일 입니다.", date1.ToString("yyyy년M월d일"), date2.ToString("yyyy년 m월 d일"), date1.Subtract(date2).Days);
            Console.WriteLine();

            Console.WriteLine("오늘 : {0}", DateTime.Today);

            DateTime y = DateTime.Today.AddDays(-1);
            Console.WriteLine("어제 : {0}", y.ToShortDateString());

            DateTime t = DateTime.Today.AddDays(1);
            Console.WriteLine("내일 : {0}", t.ToShortDateString());
            Console.WriteLine();

            Console.WriteLine("2020년은 {0}입니다.", DateTime.IsLeapYear(2020) ? "윤년" : "평년");
            Console.WriteLine("2020년 2월은 {0}일 입니다.", DateTime.DaysInMonth(2020, 2));
            Console.WriteLine();

            string date = "1990-1-27 12:6";
            DateTime aDay = DateTime.Parse(date);
            Console.WriteLine(aDay);

            string input = "1992/7/4 8:44";
            DateTime bDay;
            if(DateTime.TryParse(input, out bDay))
            {
                Console.WriteLine(bDay);
            }
            Console.WriteLine();

            DateTime d1 = DateTime.Now;
            DateTime d2 = DateTime.UtcNow;

            Console.WriteLine(d1);
            Console.WriteLine(d2);
        }
    }
}
